...
async def add_to_wishlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    upc = context.args[0]
    if wishlist_db.contains((Wishlist.user_id == user_id) & (Wishlist.upc == upc)):
        await update.message.reply_text("Item is already in your wishlist.")
        return
...
async def view_wishlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = "📝 Your wishlist UPCs:\n" + "\n".join([item["upc"] for item in items])
    await update.message.reply_text(response)

# Notify if wishlist item has markdown
async def check_and_notify_wishlist(bot, user_id: str, chat_id: int):
    ...
    if found_items:
        ...
        await bot.send_message(chat_id=chat_id, text=message)from telegram import Update
from telegram.ext import ContextTypes
from tinydb import TinyDB, Query
